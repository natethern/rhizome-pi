;;;; @(#)$Header: /u/master/rhizome/config/beos/intel/config.scm,v 1.2 2002/09/27 10:05:21 qfwfq Exp $
; $Log: config.scm,v $
; Revision 1.2  2002/09/27 10:05:21  qfwfq
...
(define-syntax cc-command-str
  (syntax-rules (static)
    ((_ static) "gcc -O2 -m486 -DRK_BIND_STATIC")
    ((_ s) "gcc -O2 -m486")))
...
(define-syntax cm-ld-command
  (syntax-rules (share)
    ((_ share libdir)
     (string-append (ld-command-str share) (if libdir (string-append " -L" libdir " -rpath " libdir) "")))
    ((_ s libdir)
     (string-append (ld-command-str s) (if libdir (string-append " -L" libdir " -Wl,-rpath," libdir) "")))))

; link library
(define-syntax ld-lib-str
  (syntax-rules (dynamic loadable)
    ((_ dynamic) "-lrhzpibe0")
    ((_ loadable) "-lrhzpibe0")
    ((_ s) "-lrhzscm -lrhzpi -lrhizome")))
...
(define-syntax cm-add-base-option
  (syntax-rules ()
    ((_ result addr)
     (string-append result " -Ttext " addr))))
