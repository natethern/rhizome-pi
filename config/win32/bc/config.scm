;;;; @(#)$Header: /u/master/rhizome/config/win32/bc/config.scm,v 1.2 2002/09/27 08:52:34 qfwfq Exp $
; $Log: config.scm,v $
; Revision 1.2  2002/09/27 08:52:34  qfwfq
; Add support of linux and lcc-win32.
;
; Revision 1.1  1999/06/15 07:14:51  qfwfq
...
(define-syntax cc-command-str
  (syntax-rules (static)
    ((_ static) "bcc32 -q -k- -O1 -w- -4 -DWIN32 -DRK_BIND_STATIC")
    ((_ s) "bcc32 -q -k- -O1 -w- -4 -DWIN32")))
...
(define-syntax ld-command-str
  (syntax-rules (con gui share)
    ((_ con) "bcc32 -q -lTpe -lap")
    ((_ gui) "bcc32 -q -lTpe -laa")
    ((_ share) "bcc32 -q -tWD")))
...
(define-syntax ld-lib-str
  (syntax-rules (dynamic loadable)
    ((_ dynamic) "rhzpibc0.lib")
    ((_ loadable) "rhzpibc0.lib")
...
(define-syntax cm-add-base-option
  (syntax-rules ()
    ((_ result addr)
     (string-append result " -lB:" addr))))

; option to specify executable name
(define-syntax output-option-str (syntax-rules () ((_) " -e")))
