;;;; @(#)$Header: /u/master/rhizome/config/unix/static/config.scm,v 1.4 2004/07/23 04:44:45 qfwfq Exp $
; $Log: config.scm,v $
; Revision 1.4  2004/07/23 04:44:45  qfwfq
; change -m option in CFLAGS
;
; Revision 1.3  2004/04/09 05:12:09  qfwfq
; ELF, W^X and pthread on OpenBSD.
;
; Revision 1.2  2002/09/27 08:37:29  qfwfq
; Add support of linux and lcc-win32.
;
; Revision 1.1  1999/06/15 07:14:41  qfwfq
...
(define-syntax cm-always-static (syntax-rules () ((_) #t)))

; C compile command
(define-syntax cc-command-str
  (syntax-rules (share)
    ((_ share) "gcc -fpic -O2 -mcpu=i686")
    ((_ s) "gcc -O2 -mcpu=i686 -DRK_BIND_STATIC")))
...
(define-syntax cm-ld-command
  (syntax-rules (share)
    ...
    ((_ s libdir)
     (string-append (ld-command-str s) (if libdir (string-append " -L" libdir) "")))))

; link library
(define-syntax ld-lib-str
  (syntax-rules (loadable)
    ((_ loadable) #f)
    ((_ s) "-lrhzscm -lrhzpi -lrhizome -lpthread -lm")))
